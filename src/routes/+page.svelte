<script>
    ...
    async function sendQuery(query) {
        ...
        try {
            ...
            history = [
                ...history,
                { role: "assistant", content: realtResponse },
            ];
    ...
    <div class="prompt-container">
        <textarea
            on:input={(e) => {
                const el = e.target;
                const shouldExpand =
                    el.scrollHeight > 60 && el.value.length > 0;
...
<div class="sidebar compact">
    <button
        ...
        style="cursor: {sidebarExpanded
            ? 'w-resize'
            : 'e-resize'}; margin-bottom: 15px; opacity: 0.7;"
    >
